...
@section('body')
    ...
    @endif
    <h1 style="font-size: 20px">DANH SÁCH ĐƠN HÀNG</h1>
    <nav>
        <ol class="breadcrumb">
    ...
    <a href="{{ route('admin.lapdonhang') }}" class="btn btn-primary">Lập đơn hàng</a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Lập đơn hàng</h5>
        ...
        <table>
            ...
            <tbody>
                ...
                @foreach ($donhang as $dh)
                    <tr>
                        <td>{{ $loop->iteration + ($donhang->currentPage() - 1) * $donhang->perPage() }}</td>
                        <td>{{ $dh->diachi->tennguoinhan }}</td>
                        {{-- <td>
                            @php
                                $tongslsp = 0;
                            @endphp
                        ...
                        <td>
                            {{ $dh->chitietdonhang->sum('soluong') }}
                        </td>

                        <td> {{ number_format($dh->tongtien, 0, ',', '.') }} VND</td>
                        <td> {{ \Carbon\Carbon::parse($dh->ngaydathang)->format('d/m/Y') }}</td>
                        <td> {{ \Carbon\Carbon::parse($dh->ngaynhanhang)->format('d/m/Y') }}</td>
                        <td>{{ $dh->trangthaidonhang->ten }}</td>
                        <td>{{ $dh->phuongthucthanhtoan->ten }}</td>



                        <td>
                            <!-- Nút Sửa -->
                            <button class="btn {{ $dh->trangthaidonhang->ten == 'Chờ xác nhận' ? 'btn-warning' : 'btn-secondary' }} "
                                {{ $dh->trangthaidonhang->ten != 'Chờ xác nhận' ? 'disabled' : '' }}>
                            <a href="{{ route('admin.update-order-status', $dh->iddh) }}" style="color: inherit; text-decoration: none;">
                                Xác nhận
                            </a>
                        </button>
                            ...
                            <!-- Nút Xóa -->
                            <form action="{{ route('admin.donhang.destroy', $dh->iddh) }}" method="POST"
                                style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#delete-modal-{{ $dh->iddh }}">Xóa</button>

                                <!-- Popup xác nhận xóa -->
                                <div class="modal fade" id="delete-modal-{{ $dh->iddh }}" tabindex="-1"
                                    aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                            ...
                            <a href="{{ route('admin.chitietdonhang', ['id' => $dh->iddh]) }}"
